...
def create_root() -> py_trees.behaviour.Behaviour:
    ...
    supervisor = py_trees.behaviours.Success(
        name="Call Supervisor to Virtually Call Elevator"
    )
    is_elevator_open.add_children(
        [
            elevator_open,
            wait_for_open_elevator,
            button_again,
            supervisor,
        ]
    )
    ...
    while not tree_success:
        try:
            ...
            if behaviour_tree.root.status == py_trees.common.Status.SUCCESS:
                ...
                reorder = input(
                    "Would you like to reorder the children of the Selector that is running? (1 for Yes 0 for No)\n"
                )
                if reorder == "1":
                    node = behaviour_tree.root
                    found = False
                    ...
                    for i in range(0, len(stored_children)):
                        ...
                        while not valid_input:
                            user_input = input(
                                "Which node (input number e.g. 0) would you like in position "
                                + str(i)
                                + "? "
                            )
                            try:
                                ...
                                print(
                                    "Please enter an integer equal to one of the node numbers."
                                )
                                continue
                            if node_number >= len(stored_children) or node_number < 0:
                                ...
                                print(
                                    "Please enter a node that hasn't been used already."
                                )
                            else:
